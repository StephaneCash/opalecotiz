...
export default function ListParticipants(props) {
    ...
    const deleteCategorie = (id) => {
        swal({
            text: "Etes-vous sûr de vouloir supprimer ce participant ?",
            buttons: true,
            dangerMode: true
        }).then((willDelete) => {
            if (willDelete) {
    ...
    return (
        <TableContainer component={Paper}>
            <div className='headTable' style={{
                ...
                <div>
                    <span>Pages</span> / <span>Participants {data && data.length > 0 ? `(${data.length})` :
                        `(0)`}</span>
                    <br />
                    <h6>Participants</h6>
                </div>
                                ...
                                <TableCell width={300}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                                        ...
                                        <div style={{
                                            ...
                                            <div style={{ fontWeight: "600", }}>{row && row.nom && row.nom && row.nom.length > 20 ?
                                                row.nom.substring(0, 20) + "..." : row && row.nom !== undefined && row.nom}</div>
